...
// ------------------------ lut creation
void mithral_lut_dense(const float* Q, int nrows, int ncols, int ncodebooks, const float* centroids, float& out_offset_sum, float& out_scale, float*  tmp_lut_f32, uint8_t* out);
...
ColMatrix<float> run_mithral(ColMatrix<float> &X, ColMatrix<float> &Q,  ColMatrix<float> &centroids,  RowVector<uint32_t> &splitdims, ColMatrix<int8_t> &splitvals, RowVector<float> &scales, RowVector<float> &offsets, RowMatrix<uint8_t> &luts);
...
namespace
{
    ...
    inline void zip_bolt_colmajor(const uint8_t* codes_in, int64_t nrows, uint32_t ncodebooks, uint8_t* codes_out)
    {
    ...
    void _compute_offsets_scale_from_mins_maxs(const __m256* mins, const __m256* maxs, int ncodebooks, float* out_offsets, float& out_offset_sum, float& out_scale)
    {
    ...
    void _dense_lut_f32_fused(const float* Q, int nrows, int ncols, int ncodebooks,
        ...
        __m256*  mins, __m256*  maxs,
        const float* centroids, float*  out_offsets,
        float& out_offset_sum, float& out_scale, float*  out)
    {
    ...
    void dense_lut_f32_fused(const float* Q, int nrows, int ncols, int ncodebooks,
        // const float* centroids, float* out)
        // SELF: above args are fast, while ones below make it like 2x slower
        const float* centroids, float*  out_offsets, float& out_offset_sum, float& out_scale, float*  out)
    {
        static constexpr int ncentroids = 16;
        static constexpr int lut_sz = ncentroids;
        ...
        _compute_offsets_scale_from_mins_maxs(mins, maxs, ncodebooks, out_offsets, out_offset_sum, out_scale);
    }


    template<int ncodebooks, int RowTileSz=1>
    void quantize_luts(const float* luts_f32, int nrows, const float* offsets, float scaleby, uint8_t* out_luts)
    {
    ...
    void quantize_luts(const float* luts_f32, int nrows, int ncodebooks, const float* offsets, float scaleby, uint8_t* out_luts)
    {
        switch (ncodebooks) 
        {
    ...
    void mithral_scan_notile(const uint8_t* codes, int64_t nblocks, const uint8_t* luts, uint8_t* dists_out)
    {
    ...
    void mithral_scan(const uint8_t* codes, int64_t nblocks, const uint8_t* luts, uint8_t* dists_out)
    {
        static_assert(NBytes > 0, "Code length <= 0 is not valid");
        static_assert(UpcastEvery % 2 == 0, "UpcastEvery must be even");
        static_assert(UpcastEvery >= 2, "UpcastEvery must be >= 2");
    ...
    void mithral_scan_notile(const uint8_t* codes, int64_t nblocks, int ncodebooks, const uint8_t* luts, uint8_t* out)
    {
        switch(ncodebooks) 
        {
    ...
    void mithral_scan(const uint8_t* codes, int64_t nblocks, int ncodebooks, const uint8_t* luts, uint8_t* out)
    {
        switch(ncodebooks) 
        {
    ...
    void mithral_scan_nochunk(const uint8_t* codes, int64_t nblocks, int ncodebooks, int noutputs, const uint8_t* luts, uint8_t* dists_out)
    {
